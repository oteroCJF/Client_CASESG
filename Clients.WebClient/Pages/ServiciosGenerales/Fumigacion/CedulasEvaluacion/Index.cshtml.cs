...
using Api.Gateway.WebClient.Proxy.Mensajeria.CedulasEvaluacion;
...
namespace Clients.WebClient.Pages.Fumigacion.CedulasEvaluacion
{
    ...
    public class IndexModel : PageModel
    {
        ...
        public List<CedulaFumigacionDto> Cedulas { get; set; }


        public IndexModel(IModuloProxy modulo, IInmuebleProxy inmuebles, ICTServicioProxy servicios,
                          IPermisoProxy permisos, IFCedulaProxy cedula)
        {
            _modulo = modulo;
        ...
        public async Task OnGet(int moduloId, int submoduloId)
        {
            ...
            if (Permisos.Where(p => p.Permiso.Nombre.Equals("Ver")).Count() != 0)
            {
                ...
                Cedulas = Anio != 0 ? await _cedula.GetCedulaByAnioAsync((int)Modulo.ServicioId, Anio, Usuario) : new List<CedulaFumigacionDto>();
            }
            else
            {

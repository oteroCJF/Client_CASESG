...
namespace Clients.WebClient.Pages.Fumigacion.CedulasEvaluacion
{
    ...
    public class DetalleModel : PageModel
    {
        ...
        public List<FIncidenciaDto> Incidencias { get; set; } = new List<FIncidenciaDto>();
        public List<CFDIDto> Facturas { get; set; } = new List<CFDIDto>();
        public List<FConfiguracionIncidenciaDto> ConfiguracionIncidencias { get; set; } = new List<FConfiguracionIncidenciaDto>();
        public CedulaFumigacionDto Cedula { get; set; } = new CedulaFumigacionDto();
        ...
        public List<CTIFumigacionDto> CTIFumigacion { get; set; } = new List<CTIFumigacionDto>();
        ...
        public DetalleModel(IModuloProxy modulo, IPermisoProxy permisos, ICTEntregableProxy centregable, ICTIncidenciaProxy cincidencias, ICTServicioProxy servicios,
                            IEstatusCedulaProxy estatusc, IEstatusEntregableProxy estatuse, IFFlujoProxy flujo, IFCFDIProxy facturas,
                            IInmuebleProxy inmuebles, IFParametroProxy variables, IFCedulaProxy cedula, IFEntregableProxy entregables,
                            IFRespuestasProxy respuestas, IFIncidenciaProxy incidencias, IFFirmanteProxy firmantes, IFRepositorioProxy facturacion,
                            IUsuarioProxy usuarios, IFLCedulaProxy logs, IFLEntregableProxy logse, IMesProxy meses, ICTIFumigacionProxy ifumigacion)
        {
            _modulo = modulo;
            _meses = meses;
        ...
        //Agregar Incidencia
        public async Task<IActionResult> OnPostCrearIncidencia([FromForm] FIncidenciaCreateCommand create)
        {
        ...
        {
            var tipos = (await _ifumigacion.GetIncidenciasByTipo(incidencia)).Select(i => i.Tipo).Distinct().ToList();

            return new JsonResult(tipos);
        }
        
        public async Task<JsonResult> OnGetTiposPO(int incidencia)
        {
            var tipos = (await _ifumigacion.GetIncidenciasByTipo(incidencia)).ToList();
        ...
        {
            CTIFumigacion = await _ifumigacion.GetNombresByTipo(tipo);

            return new JsonResult(CTIFumigacion);
        }

        public async Task<IActionResult> OnPutActualizarIncidencia([FromForm] FIncidenciaUpdateCommand update)
        {
        ...
        //Eliminar Incidencia
        public async Task<JsonResult> OnPostEliminarIncidencias([FromBody] FIncidenciaDeleteCommand delete)
        {
            int incidencias = await _incidencias.DeleteIncidencias(delete);
            return new JsonResult(incidencias);
        }

        public async Task<JsonResult> OnPostEliminarIncidencia([FromBody] FIncidenciaDeleteCommand delete)
        {
        ...
        public async Task<JsonResult> OnPostAutorizarCAE([FromBody] CedulaFumigacionDto cedula)
        {
        ...
        public async Task<JsonResult> OnPostTramiteRechazado([FromBody] CedulaFumigacionDto cedula)
        {
            List<EntregableDto> entregables = await _entregables.GetEntregablesByCedula(cedula.Id);
        ...
        public async Task<JsonResult> OnPostTramitarPago([FromBody] CedulaFumigacionDto cedula)
        {
            List<EntregableDto> entregables = await _entregables.GetEntregablesByCedula(cedula.Id);
        ...
        public async Task<IActionResult> OnGetActaEntregaRecepcion(int nfacturacion, int ninmueble, int cedulaId, string supervision)
        {
            ...
            var path = @"E:\Plantillas\Acta ER\Acta Entrega - Recepción 2022 Fumigacion.docx";
            //var path = @"C:\Users\coterog\Desktop\Acta Entrega - Recepción 2022 Fumigacion.docx";

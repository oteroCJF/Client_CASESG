...
using Api.Gateway.WebClient.Proxy.Mensajeria.Firmantes;
...
namespace Clients.WebClient.Pages.Fumigacion.Firmantes
{
    public class IndexModel : PageModel
    {
        ...
        public IndexModel(IModuloProxy modulo, ICTServicioProxy servicios, IFFirmanteProxy firmantes, IPermisoProxy permisos, 
                          IInmuebleProxy inmuebles, IUsuarioProxy usuarios)
        {
            _modulo = modulo;
        ...
        public async Task OnGet(int moduloId, int submoduloId)
        {
            ...
            if (Permisos.Where(p => p.Permiso.Nombre.Equals("Ver")).Count() != 0)
            {
                ...
                Firmantes = (await _firmantes.GetAllFirmantesAsync()).Where(f => InmueblesUsuarios.Contains(f.InmuebleId)).ToList();
            }
            else
            {
        ...
        {
            var firmantes = await _firmantes.UpdateFirmantes(firmante);

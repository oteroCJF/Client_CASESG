...
namespace Clients.WebClient.Pages.Limpieza.Facturas
{
    public class CargaFacturasModel : PageModel
    {
        ...
        public RepositorioDto Repositorios { get; set; }
        public CTServicioDto Servicio { get; set; }
        public List<PermisoUsuarioDto> Permisos { get; set; }


        public CargaFacturasModel(IModuloProxy modulo, ILRepositorioProxy RepositorioProxy, ICTServicioProxy servicios, IInmuebleProxy inmuebles,
           IPermisoProxy permisos, ILCFDIProxy facturas)
        {
            _modulo = modulo;
        ...
        public async Task OnGet(int moduloId, int submoduloId, int facturacion)
        {
            ...
            if (Permisos.Where(p => p.Permiso.Nombre.Equals("Ver")).Count() != 0)
            {
                ...
                Repositorios = await _repositorios.GetRepositorioById(facturacion);
            }
            else
            {

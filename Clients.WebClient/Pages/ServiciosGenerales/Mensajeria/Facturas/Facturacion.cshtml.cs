...
namespace Clients.WebClient.Pages.Mensajeria.Facturas
{
    public class FacturacionModel : PageModel
    {
        ...
        public FacturacionModel(IMesProxy mes, IInmuebleProxy inmuebles, IQRepositorioMensajeriaProxy repositoriosQuery, 
                                IQCFDIMensajeriaProxy cfdi, IQCedulaMensajeriaProxy cedula, IEstatusCedulaProxy estatusc, 
                                IModuloProxy modulo, IPermisoProxy permisos, ICTServicioProxy servicios, 
                                ICSoportePagoMensajeriaProxy soporteCommand)
        {
            _mes = mes;
        ...
        public async Task<JsonResult> OnPostCargarSoporte([FromForm] MSoportePagoCreateCommand soporte)
        {
            var data = await _soporteCommand.CreateIncidenciaExcel(soporte);
            return new JsonResult(data);
        }
        
        public async Task<JsonResult> OnPostActualizarSoporte([FromForm] MSoportePagoUpdateCommand soporte)
        {
            var data = await _soporteCommand.ActualizaIncidenciaExcel(soporte);
            return new JsonResult(data);
        }
    }
}
